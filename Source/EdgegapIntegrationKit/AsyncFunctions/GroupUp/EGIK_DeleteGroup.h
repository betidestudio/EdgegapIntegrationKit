...
#include "EGIK_DeleteGroup.generated.h"

USTRUCT(BlueprintType)
struct FEGIK_DeleteGroupRequest
{
	GENERATED_BODY()
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FEGIK_DeleteGroupResponse, const FEGIK_ErrorStruct&, Error);

UCLASS()
class EDGEGAPINTEGRATIONKIT_API UEGIK_DeleteGroup : public UEGIK_AsyncRequestBase
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true"), Category = "Edgegap Integration Kit | Group Up")
	static UEGIK_DeleteGroup* DeleteGroup(const FEGIK_DeleteGroupRequest& Request);

	UPROPERTY(BlueprintAssignable, Category = "Edgegap Integration Kit | Group Up")
	FEGIK_DeleteGroupResponse OnSuccess;

	UPROPERTY(BlueprintAssignable, Category = "Edgegap Integration Kit | Group Up")
	FEGIK_DeleteGroupResponse OnFailure;

	UPROPERTY(BlueprintAssignable, Category = "Edgegap Integration Kit | Group Up")
	FEGIK_DeleteGroupResponse OnRateLimited;
...
private:
	FEGIK_DeleteGroupRequest Var_Request;
};
