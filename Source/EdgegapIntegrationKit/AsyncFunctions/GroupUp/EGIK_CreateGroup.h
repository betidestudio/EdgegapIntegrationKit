...
#include "EGIK_CreateGroup.generated.h"

USTRUCT(BlueprintType)
struct FEGIK_CreateGroupRequest
{
	GENERATED_BODY()
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FEGIK_CreateGroupResponse, const FEGIK_GroupUpMemberResponse&, Response, const FEGIK_ErrorStruct&, Error);

UCLASS()
class EDGEGAPINTEGRATIONKIT_API UEGIK_CreateGroup : public UEGIK_AsyncRequestBase
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true"), Category = "Edgegap Integration Kit | Group Up")
	static UEGIK_CreateGroup* CreateGroup(const FEGIK_CreateGroupRequest& Request);

	UPROPERTY(BlueprintAssignable, Category = "Edgegap Integration Kit | Group Up")
	FEGIK_CreateGroupResponse OnSuccess;

	UPROPERTY(BlueprintAssignable, Category = "Edgegap Integration Kit | Group Up")
	FEGIK_CreateGroupResponse OnFailure;

	UPROPERTY(BlueprintAssignable, Category = "Edgegap Integration Kit | Group Up")
	FEGIK_CreateGroupResponse OnRateLimited;
...
private:
	FEGIK_CreateGroupRequest Var_Request;
};
