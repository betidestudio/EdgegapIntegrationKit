...
#include "EGIK_DeleteGroupMember.generated.h"

USTRUCT(BlueprintType)
struct FEGIK_DeleteGroupMemberRequest
{
	GENERATED_BODY()
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FEGIK_DeleteGroupMemberResponse, const FEGIK_ErrorStruct&, Error);

UCLASS()
class EDGEGAPINTEGRATIONKIT_API UEGIK_DeleteGroupMember : public UEGIK_AsyncRequestBase
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true"), Category = "Edgegap Integration Kit | Group Up")
	static UEGIK_DeleteGroupMember* DeleteGroupMember(const FEGIK_DeleteGroupMemberRequest& Request);

	UPROPERTY(BlueprintAssignable, Category = "Edgegap Integration Kit | Group Up")
	FEGIK_DeleteGroupMemberResponse OnSuccess;

	UPROPERTY(BlueprintAssignable, Category = "Edgegap Integration Kit | Group Up")
	FEGIK_DeleteGroupMemberResponse OnFailure;

	UPROPERTY(BlueprintAssignable, Category = "Edgegap Integration Kit | Group Up")
	FEGIK_DeleteGroupMemberResponse OnRateLimited;
...
private:
	FEGIK_DeleteGroupMemberRequest Var_Request;
};
