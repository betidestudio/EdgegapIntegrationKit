...
Before connecting, reserve a seat to guarantee capacity. Reservations expire when the configured reservation window elapses, so connect immediately after.
...
2. On success, read the desired port's `Link` from the saved instance's `Server` response data.
3. Connect directly using the `Link` value.
4. On failure (409 = slot full), show an error and let the player pick another server.

<Callout type="info">
The common `30s` reservation timeout is just the example shown in the server-side configuration under `seat_reservations.expiration_period`. If your configuration uses a different value, that value controls how long the client has to connect before the reservation expires.
</Callout>
...
- Reservations are short-lived. Your dedicated server must call `SB_ConfirmReservations` quickly after the player connects — see [Server Side](/server-browser/server-side).
