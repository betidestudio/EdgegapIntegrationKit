...
export function DocsLayout({
  ...
  function sidebar() {
    ...
    const nonIconLinks = links.filter((v) => v.type !== 'icon');
    const viewport = (
      <SidebarViewport>
    ...
    return (
      <>
        <SidebarContent {...rest}>
          ...
          {viewport}
          {nonIconLinks.length > 0 && (
            <div className="flex flex-col gap-1 px-4 pb-3">
              {nonIconLinks.map((item, i) => (
                <SidebarLinkItem key={i} item={item} />
              ))}
            </div>
          )}
          {(i18n || themeSwitch?.enabled !== false || footer) && (
            <div className="flex text-fd-muted-foreground items-center border-t p-4 empty:hidden">
              {footer}
              {i18n && (
                <LanguageToggle>
